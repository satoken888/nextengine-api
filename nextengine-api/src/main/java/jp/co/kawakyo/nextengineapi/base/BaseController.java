...
public abstract class BaseController {
	...
	/**
	   ...
	   * @throws InstantiationException if has errors when the specified class object cannot be instantiated
	   * @throws IllegalAccessException if has errors when does not have access to the definition of the specified class
	   */
...
//	    createUser(company.getId(), apiResponse.get(NeApiClient.KEY_ACCESS_TOKEN).toString(), apiResponse.get(NeApiClient.KEY_REFRESH_TOKEN).toString());
	  ...
	  /**
	   * Create and return Company model base on information that obtained from the API
	   *
	   * @param accessToken current Access Token of Logged-In User
	   * @param refreshToken current Refresh Token of Logged-In User
